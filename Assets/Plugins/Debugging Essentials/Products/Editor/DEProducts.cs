...
namespace DebuggingEssentials
{
    public class DEProducts
    {
        ...
        void GetSale()
        {
            ...
            // Unity 2022: need to use https for Unity 2022+ unless user enables non-secure network connections in Player Settings
            // For now disable the sales check until an https server is available in order to prevent spamming the console with warnings
            // everytime the Terrain Area is clicked
            SetPlayerPrefs();
#else
            try
            {
            ...
            {
                Debug.Log("Debugging Essentials: Sale information is unavailable. Please check the Unity Asset Store for sales.");
                SetPlayerPrefs();
            }
#endif
        }
