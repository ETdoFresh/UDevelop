    ...
    public abstract class PointPicker : MonoBehaviour
    {
        ...
        protected int GetNearestVertices(Camera camera, BaseSpline spline, Vector3 mousePosition, List<PickResult> list, float maxDistance, float distModifier)
        {
            ...
            if (!spline.ShowTerminalPoints && !spline.IsLooping)
            {
                ...
                //GK
                if (spline.gameObject.name.ToLower().Contains("cameraanimationspline"))// ||
                                                                                       //spline.gameObject.GetComponent<CameraAnimationSplineRenderer>())//spline.GetType() == typeof(IDP.CameraAnimationSpline))
                {
                    //points = points.Take(spline.ControlPointCount-1);//.Skip(0);
                    points = GetLocalPoints(spline);
                    points = points.Take(spline.ControlPointCount - 1);//.Skip(0);
                    index--;
                }
            }
