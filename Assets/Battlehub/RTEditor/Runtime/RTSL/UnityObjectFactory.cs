...
namespace Battlehub.RTSL
{
    public class UnityObjectFactory : IUnityObjectFactory
    {
        ...
        public UnityObject CreateInstance(Type type, IPersistentSurrogate surrogate)
        {
            ...
            {
                if (surrogate != null && surrogate.CanInstantiate(type))
                {
                    return (UnityObject)surrogate.Instantiate(type);
                }
            ...
            {
                Debug.LogError(e);
                Debug.LogWarning("Collecting scene dependencies could fix this exception. Tools->Runtime Save Load->Collect Scene Dependencies");
                return null;
            }
        }
    }
