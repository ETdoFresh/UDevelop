...
#pragma warning restore CS0612
        ...
        protected virtual void Update()
        {
            if (m_isDragging)
            {
                ...
                {
                    Vector3 offset = GetDragOffset(point, m_prevPoint);
...
#pragma warning restore CS0612
        ...
        protected virtual int Hit(Vector2 pointer)
        {
            return Hit(pointer, HandlesPositions, HandlesNormals);
        }

        protected virtual int Hit(Vector2 pointer, Vector3[] vertices, Vector3[] normals)
        {
            ...
            for (int i = 0; i < vertices.Length; ++i)
            {
                ...
                if(distance < minMag && distance <= SelectionMargin * GizmoUtility.HandleScale)
                {
                    minMag = distance;
                    index = i;
                }
        ...
        protected virtual Vector3 GetDragOffset(Vector3 point, Vector3 prevPoint)
        {
            Vector3 offset = m_handlesInverseTransform.MultiplyVector(point - prevPoint);
