...
namespace Battlehub.RTEditor.ViewModels
{
    ...
    public class SelectObjectViewModel : HierarchicalDataViewModel<ProjectItem>, ISelectObjectDialog
    {
        ...
        protected virtual async Task LoadDataAsync()
        {
            ...
            /*
            HashSet<UnityObject> sceneObjectDepsHs = new HashSet<UnityObject>();
            var findSceneObjectDepsTasks = new List<Task>();
            */
            for (int i = 0; i < sceneObjects.Length; ++i)
            {
                ...
                /*
                else if (m_project.Utils.ToGuid(ObjectType) != Guid.Empty)
                {
                    Component[] components = exposeToEditor.gameObject.GetComponents<Component>();
                    foreach(Component component in components)
                    {
                        findSceneObjectDepsTasks.Add(FindDeepSceneDependencies(component, sceneObjectDepsHs));
                    }
                }
                */
            }
            ...
            /*
            await Task.WhenAll(findSceneObjectDepsTasks);

            foreach(UnityObject dep in sceneObjectDepsHs)
            {
                AddSceneObject(sceneCache, dep);
            }
            */
        ...
        /*
        private async Task FindDeepSceneDependencies(Component component, HashSet<UnityObject> sceneObjectDepsHs)
        {
            object[] deps = await m_project.Utils.FindDeepDependenciesAsync(component);
            if (deps == null)
            {
                return;
            }
            ...
            {
                if (dep.GetType() == ObjectType && dep is UnityObject)
                {
                    sceneObjectDepsHs.Add((UnityObject)dep);
                }
            }
        }
        */

        private void AddSceneObject(List<ProjectItem> sceneCache, UnityObject obj)
        {
            if (m_project.Utils.ToProjectItem(obj) != null)
            {
                return;
            }
            ProjectItem assetItem = m_project.Utils.CreateAssetItem(m_project.Utils.ToGuid(obj.GetType()), obj.name, m_project.Utils.GetExt(obj.GetType()));
            m_project.Utils.SetPersistentID(assetItem, m_project.Utils.InstanceIDToPersistentID(obj));
