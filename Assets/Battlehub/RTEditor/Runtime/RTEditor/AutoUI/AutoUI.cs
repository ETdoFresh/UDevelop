...
namespace Battlehub.RTEditor.UI
{
    public partial class AutoUI : IDisposable
    {
        ...
        public Transform CreateDialog(object viewModel, string sTitle = "", params object[] dependencies)
        {
            return CreateDialog(250, 250, 700, 400, true, viewModel, sTitle, dependencies);
        }

        public Transform CreateDialog(float width, float height, object viewModel, string sTitle = "", params object[] dependencies)
        {
            return CreateDialog(width, height, false, viewModel, sTitle, dependencies);
        }

        public Transform CreateDialog(float width, float height, bool canResize, object viewModel, string sTitle = "", params object[] dependencies)
        {
            return CreateDialog(width, height, width, height, canResize, viewModel, sTitle, dependencies);
        }

        public Transform CreateDialog(float minWidth, float minHeight, float preferredWith, float preferredHeight, bool canResize, object viewModel, string sTitle = "", params object[] dependencies)
        {
            ...
            //GK Transform layoutRoot = wm.CreateDialogWindow(RuntimeWindowType.EmptyDialog.ToString(), "Auto UI Example", okAction, cancelAction, minWidth, minHeight, preferredWith, preferredHeight, canResize);
            Transform layoutRoot = wm.CreateDialogWindow(RuntimeWindowType.EmptyDialog.ToString(), sTitle, okAction, cancelAction, minWidth, minHeight, preferredWith, preferredHeight, canResize);
            Dialog dialog = layoutRoot.GetComponentInParent<Dialog>();
            //GK
            dialog.Icon = Resources.Load<Sprite>("None");
            //IRTEAppearance appearance = IOC.Resolve<IRTEAppearance>();
            //layoutRoot.transform.localScale *= appearance.UIScale;
